...
## Story R1.7-14: Vergleichswerte (Ø/Best) backendseitig zentral berechnen (Overview + Peak + Segment) ✅ done
**Als** Nutzer
...
### Acceptance Criteria
- [x] Backend liefert Vergleichswerte für **Overview und Peak Demand** aus einem zentralen Vergleichsservice, statt aus Frontend-Berechnungen.
- [x] Vergleichswerte sind nach **Session Type** getrennt (`Match`, `Training`, …) und verwenden denselben Scope in allen Views.
- [x] Segmentkontext wird unterstützt: z. B. `Match-Halbzeit 1`, `Training-Spielform`, `Training-Athletics` (falls Segmentdaten vorhanden), andernfalls transparent `nicht verfügbar`. Das bedeutet dass Segment Categories nur innerhalb des selben Session Typs vergleichbar sind. Das bedeutet mehrere "Training" Sessions vom Typ Spielform tragen zum Vergleich bei, eine Categorie Spielform bei einer z.B.  Warmup-Session trägt hier nicht zum Vergleich bei. 
- [x] Für jede vergleichbare Metrik werden mindestens `averageLastN` und `best` serverseitig bereitgestellt.
- [x] Das Profil enthält eine Einstellung `comparisonSessionsCount` (Anzahl berücksichtigter Sessions), **Default = 5**.
...
- [x] Änderungen an `comparisonSessionsCount` wirken in Overview und Peak einheitlich, ohne UI-Neuberechnung.
- [x] wenn weniger als comparisonSessionsCount Sessions vorliegen, dann werden alle zur Verfügung stehenden Sessions verwendet
- [x] der Durchschnitt und Best ist immer auf Basis der letzten Session (bis comparisonSessionsCount), auch wenn man beispielsweise die 20. letzte session betrachtet.
- [x] Die Session mit dem höchsten Session Datum zählt auch mit rein
- [x] API-/DTO-Dokumentation beschreibt klar, welche Felder Peak-basiert und welche Overview-basiert sind.
- [x] Bei fehlender Datenbasis oder Qualitätsproblemen liefert die API erklärbare Availability-Flags statt impliziter Nullwerte.
