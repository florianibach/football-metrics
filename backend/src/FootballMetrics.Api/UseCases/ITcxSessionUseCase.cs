...
public interface ITcxSessionUseCase
{
    ...
    Task<TcxUpload?> AddSegmentAsync(Guid id, string label, int startSecond, int endSecond, string? reason, string? category, CancellationToken cancellationToken);
    Task<TcxUpload?> UpdateSegmentAsync(Guid id, Guid segmentId, string? label, int? startSecond, int? endSecond, string? reason, string? category, CancellationToken cancellationToken);
